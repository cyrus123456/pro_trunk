  ...
  <div class="app-container">
    <div class="app-container-bg">
      <section class="tabs-content-item">
        <div class="pageName">{{$route.name === 'oaAutoClaimsUnderwrit' ? '总公司' : '分公司'}}核赔权限申请</div>
        ...
        <div class="table-base table-select-width-full">
          ...
          <el-table :data="results"  style="width: 100%;margin-bottom: 20px;"
            ...
            <el-table-column prop="comcode" label="登录机构" width="150">
            ...
            <el-table-column prop="comcname" label="机构名称"  min-width="260">
              <template slot-scope="scope">
            ...
            <el-table-column prop="usercode" label="工号"  width="150">
              <template slot-scope="scope">
                <el-select
            ...
            <el-table-column prop="usercname" label="姓名"  width="150">
              <template slot-scope="scope">
                <el-select
            ...
            <el-table-column prop="templateid" label="模板ID" width="150">
              <template slot-scope="scope">
                <el-select
                  @change="(value)=>{selectChangeTemplateId(value,scope.$index, scope.row)}"
                  :disabled ="!docStatus"
                  ...
                  >
                  <el-option
                    v-for="(item,index) in scope.row.optionsTemplateId"
                    :key="item.value+index+scope.$index"
                    :label="item.label"
                    :value="item.value">
                  </el-option>
            ...
            <el-table-column prop="templatename" label="模板名称" width="150">
              <template slot-scope="scope">
                <el-input v-model="scope.row.templatename" placeholder="" disabled></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="gradecode" label="岗位代码"  width="120">
              <template slot-scope="scope">
                <el-select
                  ...
                  @focus="(value)=>{visibleChangeGradeCode(value,scope.$index, scope.row)}"
                  placeholder="请输入关键词"
                  :loading="scope.row.loadingGradeCode">
                  <el-option
                    v-for="(item,index) in scope.row.optionsGradeCode"
                    :key="item.value+index+scope.$index"
                    :label="item.label"
                    :value="item.value">
                  </el-option>
              ...
              <template slot-scope="scope">
                <!-- <el-tooltip effect="dark" :content="scope.row.gradename"  placement="top">
                  <el-select
                    ...
                    <el-option
                      v-for="(item,index) in scope.row.optionsGradeName"
                      :key="item.value+index+scope.$index"
                      :label="item.label"
                      :value="item.value">
                    </el-option>
                ...
                <el-input v-model="scope.row.gradename" placeholder="" disabled></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="nodeno" label="节点" width="150">
              <template slot-scope="scope">
                <el-select
                  @change="(value)=>{selectChangeNodeNo(value,scope.$index, scope.row)}"
                  :disabled ="!docStatus"
                  v-model="scope.row.nodeno"
                  @focus="(value)=>{visibleChangeNodeNo(value,scope.$index, scope.row)}"
                  placeholder="请选择"
                  >
                  <el-option
                    v-for="(item,index) in scope.row.optionsNodeNo"
                    :key="item.value+index+scope.$index"
                    :label="item.label"
                    :value="item.value">
                  </el-option>
            ...
            <el-table-column prop="classname" label="险种" width="200">
              <template slot-scope="scope">
                  <el-row >
...
<script src="./oaAutoUnderwritApply.js"></script>
