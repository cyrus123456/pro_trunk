...
export default {
  ...
  methods: {
    initPage () {
      if (getToken('oaAutoplatformpostReplaceApplyTask') && getToken('oaAutoplatformpostReplaceApplyTask').length > 0) {
        // this.results = getToken('oaAutoplatformpostReplaceApplyTask')
        this.results = getToken('oaAutoplatformpostReplaceApplyTask').map(item => {
          return {
    ...
    commitData () {
      ...
      dualCorePermissions({
        ...
        if (res.flag === '1') {
          // this.$confirm('权限申请岗位换人申请提交成功, 是否退出页面?', '提示', {
          ...
          //   this.closePage()
          // })
          this.$message.success('权限申请岗位换人申请提交成功')
          setTimeout(() => {
            this.closePage()
          }, 3000)
        }
