...
export default {
  ...
  methods: {
    initPage (callback) {
      ...
      if (getToken('oaAutoEhrBand') && getToken('oaAutoEhrBand').length > 0) {
        getToken('oaAutoEhrBand').forEach(item => {
    ...
    handleTemplateDownload () {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['人力工号', '系统代码', '工号']
        const filterVal = ['hrno', 'systemcode', 'usercode']
        const list = [
          {
            'hrno': 'B201902060001',
            'systemcode': 'P027',
    ...
    commitData () {
      ...
      this.results.forEach((item, index) => {
        ...
        postData.push({
          ...
          hrno: postDataItem.hrno,
          systemcode: postDataItem.systemcode,
          systemname: postDataItem.systemname,
    ...
    selectChangeSystemCode (value, index, row) {
      ...
      for (let i = 0; i < row.optionsSystemCode.length; i++) {
        if (row.optionsSystemCode[i].value === value) {
          row.systemname = row.optionsSystemCode[i].attachData.split('--')[1]
          row.optionsSystemName = [{ value: row.optionsSystemCode[i].attachData.split('--')[1], label: row.optionsSystemCode[i].attachData.split('--')[1] }]
          return false
        }
      }
    },
    selectChangeSystemName (value, index, row) {
      ...
      // this.resetUserName(index, row)
      // this.resetUserCode(index, row)
      for (let i = 0; i < row.optionsSystemName.length; i++) {
        if (row.optionsSystemName[i].value === value) {
          row.systemcode = row.optionsSystemName[i].attachData.split('--')[0]
          row.optionsSystemCode = [{ value: row.optionsSystemName[i].attachData.split('--')[0], label: row.optionsSystemName[i].attachData.split('--')[0] }]
          return false
        }
      }
    },
    ...
    remoteMethodSystemCode (query, index, row) {
      /* if (!row.comcode){
        this.$message({
          message: '请先选择登录机构！',
          type: 'warning',
      ...
      if (query !== '') {
        row.loadingSystemCode = true
        systemRemarkCode({
          actionType: 'query',
          codeType: 'systemRemarkCode',
          fieldValue: query
        }).then(res => {
          console.log('pageContent', res)
          row.loadingSystemCode = false
          if (res.codeLabels && res.codeLabels.length > 0) {
            row.optionsSystemCode = res.codeLabels.map(item => {
              // return { value: item.split('--')[0], label: item }
    ...
    remoteMethodSystemName (query, index, row) {
      /* if (!row.comcode){
        this.$message({
          message: '请先选择登录机构！',
          type: 'warning',
      ...
      if (query !== '') {
        ...
        systemRemarkCode({
          ...
          console.log('pageContent', res)
          row.loadingSystemName = false
          if (res.codeLabels && res.codeLabels.length > 0) {
            row.optionsSystemName = res.codeLabels.map(item => {
    ...
    unloadFn () {
      ...
      //   // 关闭窗口
      // }
      setToken('reloadFlag', {name: 'oaAutoEhrBand'})
    },
    beforeunloadFn () {
